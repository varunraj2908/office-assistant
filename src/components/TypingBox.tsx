...
// export const TypingBox = ({ handleSpeak }: TypingProps) => {
//   const loading = useAITeacher((state) => state.loading);
//   const [question, setQuestion] = useState("");
//   const [mouthShape, setMouthShape] = useState<string | number>("mouthA");

//   const ask = () => {
//     if (!question.trim()) return;

//     const text = "Hello good morning";
//     const visemes = generateFakeVisemes(text);
//     const audioPlayer = createFakeAudioPlayer(text);
...
//   useEffect(() => {
//     const current = useAITeacher.getState().currentMessage;
//     const audioPlayer = current?.audioPlayer;
//     const visemes = current?.visemes || [];
...
//   }, [useAITeacher.getState().currentMessage]);
...
import AIavatar from "../Aiavtar.json";
import { useState, useEffect } from "react";
import { Message, FakeAudioPlayer } from "./../../src/types/Message";
...
export const TypingBox = ({ handleSpeak }: TypingProps) => {
  ...
  return (
    <div className="absolute w-full bottom-0 flex px-5 pt-3 flex-col bg-red-500/50 z-50">
      {loading ? (
        <div className="flex justify-center items-center">
          <span className="relative flex h-4 w-4">
        ...
        <div className="gap-3 flex">
          <input
            className="flex-grow bg-slate-800/60 p-2 px-4 text-white placeholder:text-white/50 border border-white shadow-inner shadow-slate-900/60"
            placeholder="Ask Something..."
            value={question}
            onChange={(e) => setQuestion(e.target.value)}
          ...
          <button className="ask-btn" onClick={ask}>
            <Lottie animationData={AIavatar} loop className="w-6 h-6" />
            Ask To Assistant
          </button>
          <button className="ask-btn" onClick={handleSpeak}>
            <Lottie animationData={AIavatar} loop className="w-6 h-6" />
            Ask to AI
          </button>
        </div>
      ...
      <div className="text-center mt-4">
        <div className={`mouth-shape ${mouthShape}`} />
      </div>
    </div>
  );
};

function generateFakeVisemes(text: string): [number, string | number][] {
  const phonemes = ["mouthA", "mouthO", "mouthU", "mouthE", "mouthI", 0, 1, 2, 3, 4, 5];
  return text.split("").map((_, i) => [i * 150, phonemes[i % phonemes.length]]);
}

function createFakeAudioPlayer(text: string): FakeAudioPlayer {
  ...
  utterance.voice =
    voices.find((v) => v.lang === "en-IN" && /female|zira|heli/i.test(v.name)) ||
    voices.find((v) => v.lang.startsWith("en") && /female|zira|heli/i.test(v.name)) ||
    voices[0];

  const fakePlayer: FakeAudioPlayer = {
    currentTime: 0,
    play: () => {
      ...
      utterance.onend = () => {
        ...
        useAITeacher.setState(() => ({ currentMessage: null }));
      };
...
// export const TypingBox = ({ handleSpeak}: TypeingProps) => {
